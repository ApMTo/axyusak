...
// clientWidth, clientHeight - показывает ширину и высоту
// scrollTop, scrollLeft - Показывает скроллы по пиксели
// offsetLeft,Top. offsetWith,height
...
myFunction(); // Вызываем функцию, чтобы увеличить res
var innerFunction = myFunction(); // Получаем внутреннюю функцию
// innerFunction(); // Вызываем внутреннюю функцию, которая должна вывести "Hello"

// let num1 = +prompt('Введите число');

// if(isNaN(num1) || num1 > 10) {
//   console.log('Вы не ввели цифру, или же вы написали цифру, которая больше 11.');
// } else {
//   for(let i = 0; i <= 10; i++) {
...
btnInput.addEventListener("click", function () {
  let inputNumber = parseInt(inputnum.value);
  if (inputnum.value > 10) {
    let liElement = document.createElement("li");
    liElement.innerHTML = "Դուք մուտքագրել եք 10-ից մեծ թիվ";
    numbers.append(liElement);
    inputnum.value = "";
  } else if (inputnum.value < 1) {
    let liElement = document.createElement("li");
    liElement.innerHTML = "Դուք մուտքագրել եք 1-ից փոքր թիվ";
    numbers.append(liElement);
    inputnum.value = "";
  } else {
    for (let i = 1; i <= 10; i++) {
      let liElement = document.createElement("li");
      liElement.innerHTML = `${inputNumber} * ${i} = ${inputNumber * i}`;
      numbers.append(liElement);
      inputnum.value = "";
    }
  }
